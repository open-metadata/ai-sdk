...
pub async fn run_create_agent_tui(profile: &str) -> CliResult<()> {
    // Setup terminal
    ...
    // Run the main loop
    let result = run_wizard_loop(&mut terminal, &mut wizard, profile).await;
    ...
    loop {
            ...
            && !wizard.status.is_loading()
        {
            ...
            tokio::spawn(async move {
                let result = load_personas(&profile).await;
        ...
        if wizard.step == Step::Actions && !wizard.bots_loaded && !wizard.status.is_loading() {
            ...
            let profile = profile.to_string();
            tokio::spawn(async move {
                let result = load_bots(&profile).await;
...
async fn load_personas(profile: &str) -> Result<Vec<PersonaInfo>, String> {
...
/// Load bots from API.
async fn load_bots(profile: &str) -> Result<Vec<BotInfo>, String> {
    let config = ResolvedConfig::load(profile).map_err(|e| e.to_string())?;
    let client = AISdkClient::new(&config).map_err(|e| e.to_string())?;
