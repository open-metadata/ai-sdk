    ...
    let result = match cli.command {
        Commands::Configure { action } => match action {
            Some(ConfigureAction::Set { key, value }) => {
                commands::configure::run_set(&cli.profile, &key, &value)
            }
            Some(ConfigureAction::Get { key }) => commands::configure::run_get(&cli.profile, &key),
            Some(ConfigureAction::List) => commands::configure::run_list(&cli.profile),
            Some(ConfigureAction::ListProfiles) => commands::configure::run_list_profiles(),
            Some(ConfigureAction::RemoveProfile { name }) => {
                commands::configure::run_remove_profile(&name)
            }
            None => commands::configure::run_interactive(&cli.profile),
        },

        Commands::Agents { action } => match action {
            AgentsAction::List => commands::agents::run_list(&cli.profile).await,
            AgentsAction::Info { name } => commands::agents::run_info(&cli.profile, &name).await,
            AgentsAction::Create {
                name,
                persona,
                description,
                ...
                match (name, description, persona) {
                    ...
                    _ => {
                        // Launch TUI wizard
                        tui::run_create_agent_tui(&cli.profile).await
                    }
                }
            }
        },
        ...
        Commands::Bots { action } => match action {
            BotsAction::List { limit, json } => {
                commands::bots::run_list(&cli.profile, limit, json).await
            }
            BotsAction::Get { name, json } => {
                commands::bots::run_get(&cli.profile, &name, json).await
            }
        },

        Commands::Personas { action } => match action {
            PersonasAction::List { limit, json } => {
                commands::personas::run_list(&cli.profile, limit, json).await
            }
            PersonasAction::Get { name, json } => {
                commands::personas::run_get(&cli.profile, &name, json).await
            }
            PersonasAction::Create {
                ...
                match (name, description, prompt) {
                    ...
                    _ => {
                        // Launch TUI wizard
                        tui::run_create_persona_tui(&cli.profile).await
                    }
                }
            }
        },
