...
Every difference you see in the output is produced server-side by OpenMetadata:
    ...
    * AI Entity Context   (get_asset_context)    — the full "Context Profile"
      of a single asset, filtered live by the caller's RBAC. The Compliance
      Officer sees PII column profiles and sample values; the Data Engineer
      gets the same asset with those columns masked.
...
Prerequisites
--------------------------------------------------------------------------------
1. An OpenMetadata 2.0 instance with the banking-redshift demo data loaded and
   the persona context + RBAC policies applied. Run ``setup_demo.py`` first.
...
def build_agent(client: AISdk, model: str):
    ...
    langchain_tools = client.mcp.as_langchain_tools(include=DEMO_TOOLS)
    return create_agent(
        model=model,
...
def raw_asset_context(client: AISdk, entity_type: str, fqn: str) -> str:
    ...
    result = client.mcp.call_tool(
        MCPTool.GET_ASSET_CONTEXT,
        {"entityType": entity_type, "fqn": fqn, "format": "markdown"},
    )
    if not result.success or result.data is None:
...
def resolve_table_fqn(client: AISdk, table: str, explicit: str | None) -> str | None:
    ...
    if not result.success or not isinstance(result.data, dict):
        return None
    hits = result.data.get("results") or result.data.get("hits") or result.data.get("data")
...
def print_diff(left: str, right: str, left_label: str, right_label: str) -> None:
    """Show a unified diff so masked / omitted lines are obvious."""
    ...
    if body.strip():
        ...
        print("(identical — did setup_demo.py apply the persona rules and PII policy?)")
    ...
    if use_raw:
        ...
        if fqn is None:
            print(
                ...
                "(e.g. redshift.banking.marts_core.dim_customers)."
            )
            return
        print(f"\nComparing get_asset_context for: {fqn}")
...
def _require_env(name: str) -> str:
    ...
    if not value:
        ...
        print("See the module docstring for the full list of required variables.", file=sys.stderr)
        raise SystemExit(1)
    return value
...
def _load_identities() -> tuple[str, Identity, Identity]:
    ...
    compliance = Identity("David Kim", "ComplianceOfficer", _require_env("DEMO_COMPLIANCE_TOKEN"))
    engineer = Identity("Sara Johnson", "DataEngineer", _require_env("DEMO_ENGINEER_TOKEN"))
...
def main() -> None:
    ...
    clients = {compliance_id.label: compliance_client, engineer_id.label: engineer_client}

    try:
        compliance_agent = build_agent(compliance_client, args.model) if args.agent else None
        engineer_agent = build_agent(engineer_client, args.model) if args.agent else None
