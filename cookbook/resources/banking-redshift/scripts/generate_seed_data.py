"""Deterministic seed-data generator for the banking-redshift demo.
...
def write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        writer.writeheader()
        for row in rows:
            normalised = {k: ("" if v is None else v) for k, v in row.items()}
...
def main() -> None:
    ...
    logger.info("Generating banking demo seed data (seed=%d)", SEED)
