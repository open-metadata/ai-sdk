#!/usr/bin/env bash
#
# Provision the local Superset instance for the banking-redshift demo:
#   1. Verify Redshift connection env vars are set.
#   2. Wait for Superset's /health endpoint to be reachable.
#   3. Sanity-check the admin login.
#   4. Hand off to `setup_superset_charts.py`, which creates the Redshift
