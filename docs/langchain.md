...
tool = AiSdkAgentTool.from_agent(agent_handle)
```
...
3. The agent uses the ID to maintain context
...
For async LangChain pipelines, enable async on the AiSdk client:
...
- Ensure `enable_async=True` on the AiSdk client
