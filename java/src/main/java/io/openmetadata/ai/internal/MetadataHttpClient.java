...
public class MetadataHttpClient implements AutoCloseable {
    ...
    public MetadataHttpClient(String host, String token, Duration timeout, int maxRetries, Duration retryDelay) {
    ...
    /**
     * Lists all API-enabled agents with pagination.
    ...
    /**
     * Lists API-enabled agents with optional limit.
     ...
     */
    public void stream(String agentName, InvokeRequest invokeRequest, Consumer<StreamEvent> eventConsumer) {
        String encodedName = URLEncoder.encode(agentName, StandardCharsets.UTF_8);
        String requestBody;
        try {
            requestBody = objectMapper.writeValueAsString(invokeRequest);
    ...
    /**
     * Invokes an agent with streaming, returning a Stream of events.
    ...
    /**
     * Lists bots with optional limit.
     * Automatically paginates through all results.
     *
    ...
    /**
     * Lists personas with optional limit.
     * Automatically paginates through all results.
     *
    ...
    public PersonaInfo getPersonaByName(String name) {
        ...
        HttpResponse<String> response = executeWithRetryForResource(request, ResourceType.PERSONA, name);
        try {
    ...
    public PersonaInfo createPersona(CreatePersonaRequest createRequest) {
        ...
        HttpResponse<String> response = executeWithRetryForResource(request, ResourceType.PERSONA, null);
        try {
            return objectMapper.readValue(response.body(), PersonaInfo.class);
        } catch (JsonProcessingException e) {
    ...
    /**
     * Lists abilities with optional limit.
     * Automatically paginates through all results.
     *
    ...
    public List<AbilityInfo> listAbilities(Integer limit) {
        return paginateList(host + ABILITIES_API_PATH, limit, new TypeReference<List<AbilityInfo>>() {});
    }
    ...
    public AbilityInfo getAbilityByName(String name) {
        ...
        HttpResponse<String> response = executeWithRetryForResource(request, ResourceType.ABILITY, name);
        try {
    ...
    private enum ResourceType {
        AGENT, BOT, PERSONA, ABILITY
    }

    private HttpResponse<String> executeWithRetryForResource(HttpRequest request, ResourceType resourceType, String resourceName) {
        int attempts = 0;
    ...
    private void handleErrorStatusForResource(int statusCode, ResourceType resourceType, String resourceName) {
        switch (statusCode) {
            case 200:
            case 201:
                return;
